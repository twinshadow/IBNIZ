...
void
gen_nativerun(void *a) {
}

/* register moves */

void
gen_mov_reg_reg_reg(int t, int s) {
	printf("%c=%c;\n", t + 'A', s + 'A');
}

void
gen_mov_reg_imm(int t, int32_t i) {
	printf("%c=0x%X;\n", t + 'A', i);
}

void
gen_mov_reg_ivar(int r, int v) {
	printf("%c=%s;\n", r + 'A', ivarnames[v]);
}

void
gen_mov_ivar_reg(int v, int r) {
	printf("%s=%c;\n", ivarnames[v], r + 'A');
}
...
void
gen_load_reg_reg(int t, int a) {
	printf("%c=mem[ROR(%c,16)&0xFFFFF];\n", t + 'A', a + 'A');
}

void
gen_load_reg_imm(int t, int32_t a) {
	printf("%c=mem[0x%X];\n", t + 'A', ROR(a, 16) & 0xFFFFF);
}

void
gen_store_reg_reg(int a, int s) {
	printf("mem[ROR(%c,16)&0xFFFFF]=%c;\n", a + 'A', s + 'A');
}

void
gen_store_reg_imm(int a, int32_t s) {
	printf("mem[ROR(%c,16)&0xFFFFF]=0x%X;\n", a + 'A', s);
}

void
gen_store_imm_reg(int32_t a, int s) {
	printf("mem[0x%X]=%c;\n", ROR(a, 16) & 0xFFFFFF, s + 'A');
}

void
gen_store_imm_imm(int32_t a, int32_t s) {
...
void
gen_add_reg_reg_reg(int t, int s1, int s) {
	printf("%c=%c+%c;\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_add_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=%c+0x%X;\n", t + 'A', s1 + 'A', i);
}

void
gen_sub_reg_reg_reg(int t, int s1, int s) {
	printf("%c=%c-%c;\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_sub_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=%c-0x%X;\n", t + 'A', s1 + 'A', i);
}

void
gen_mul_reg_reg_reg(int t, int s1, int s) {
	printf("%c=IBNIZ_MUL(%c,%c);\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_mul_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=IBNIZ_MUL(%c,0x%X);\n", t + 'A', s1 + 'A', i);
}

void
gen_div_reg_reg_reg(int t, int s1, int s) {
	printf("%c=IBNIZ_DIV(%c,%c);\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_div_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=IBNIZ_DIV(%c,0x%X);\n", t + 'A', s1 + 'A', i);
}

void
gen_mod_reg_reg_reg(int t, int s1, int s) {
	printf("%c=IBNIZ_MOD(%c,%c);\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_mod_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=IBNIZ_MOD(%c,0x%X);\n", t + 'A', s1 + 'A', i);
}

void
gen_and_reg_reg_reg(int t, int s1, int s) {
	printf("%c=%c&%c;\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_and_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=%c&0x%X;\n", t + 'A', s1 + 'A', i);
}

void
gen_or_reg_reg_reg(int t, int s1, int s) {
	printf("%c=%c|%c;\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_or_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=%c|0x%X;\n", t + 'A', s1 + 'A', i);
}

void
gen_xor_reg_reg_reg(int t, int s1, int s) {
	printf("%c=%c^%c;\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_xor_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=%c^0x%X;\n", t + 'A', s1 + 'A', i);
}

void
gen_ror_reg_reg_reg(int t, int s1, int s) {
	printf("%c=IBNIZ_ROR(%c,%c);\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_ror_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=IBNIZ_ROR(%c,0x%X);\n", t + 'A', s1 + 'A', i);
}

void
gen_shl_reg_reg_reg(int t, int s1, int s) {
	printf("%c=IBNIZ_SHL(%c,%c);\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_shl_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=IBNIZ_SHL(%c,0x%X);\n", t + 'A', s1 + 'A', i);
}

void
gen_neg_reg_reg(int t, int s) {
	printf("%c=~%c;\n", t + 'A', s + 'A');
}

void
gen_atan2_reg_reg_reg(int t, int s1, int s) {
	printf("%c=IBNIZ_ATAN2(%c,%c);\n", t + 'A', s1 + 'A', s + 'A');
}

void
gen_atan2_reg_reg_imm(int t, int s1, int32_t i) {
	printf("%c=IBNIZ_ATAN2(%c,0x%X);\n", t + 'A', s1 + 'A', i);
}

void
gen_sin_reg_reg(int t, int s) {
	printf("%c=IBNIZ_SIN(%c);\n", t + 'A', s + 'A');
}

void
gen_sqrt_reg_reg(int t, int s) {
	printf("%c=IBNIZ_SQRT(%c);\n", t + 'A', s + 'A');
}

void
gen_isneg_reg_reg(int t, int s) {
	printf("%c=IBNIZ_ISNEG(%c);\n", t + 'A', s + 'A');
}

void
gen_ispos_reg_reg(int t, int s) {
	printf("%c=IBNIZ_ISPOS(%c);\n", t + 'A', s + 'A');
}

void
gen_iszero_reg_reg(int t, int s) {
...
void
gen_beq_reg_lab(int s, int l) {
	printf("if(!%c) goto l%d;\n", s + 'A', l);
}

void
gen_bne_reg_lab(int s, int l) {
	printf("if(%c) goto l%d;\n", s + 'A', l);
}

void
gen_beq_reg_rstack(int s) {
	printf("if(!%c) goto *((void*)main+rstack[rsp]);\n", s + 'A');
}

void
gen_bne_reg_rstack(int s) {
	printf("if(%c) goto *((void*)main+rstack[rsp]);\n", s + 'A');
}
